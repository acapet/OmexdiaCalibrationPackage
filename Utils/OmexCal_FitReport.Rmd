...
steptable<-data.frame(Step     = NA,
  ...
  for (ifit in c(1:length(PLIST))){
    steptable[ifit,"Step"]        <- stepNames[ifit]
    try ( steptable$Profiles[ifit]      <- paste(VLIST[[ifit]],sep ="\n"), silent = TRUE)
    try ( steptable$Fluxes[ifit]        <- paste(FLIST[[ifit]],sep ="\n"), silent = TRUE)
    try ( steptable$MicroProfiles[ifit] <- paste(MLIST[[ifit]],sep ="\n"), silent = TRUE)
    try ( steptable$Parameters[ifit]    <- paste(PLIST[[ifit]],sep ="\n"), silent = TRUE)
  }

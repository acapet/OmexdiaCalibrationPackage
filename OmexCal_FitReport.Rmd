  ...
  for (jr in (1:nrow(allpdfs))){
    ...
    if ((rownames(allpdfs)[jr]%in% unlist(PLIST)) &( allpdfs[jr,ir]!=" \" ")){
      if (abs( as.numeric(allpdfs[jr,ir])- as.numeric(allpdfs[jr,"min"]))
          /( .5*(as.numeric(allpdfs[jr,ir])+as.numeric(allpdfs[jr,"min"])))<0.02){
        allpdfs[jr,ir]    <- cell_spec( allpdfs[jr,ir] ,"latex", color ="red")
        #allpdfs[jr,"min"] <- cell_spec( allpdfs[jr,"min"] ,"latex", color ="red")
        } 
      else if (abs( as.numeric(allpdfs[jr,ir])- as.numeric(allpdfs[jr,"max"]))/(
        .5*(as.numeric(allpdfs[jr,ir])+as.numeric(allpdfs[jr,"max"])))<0.02){
        allpdfs[jr,ir]    <- cell_spec( allpdfs[jr,ir] ,"latex", color ="red")
        #allpdfs[jr,"max"] <- cell_spec( allpdfs[jr,"max"] ,"latex", color ="red")
      }
    }
...
The variables costs ares estimated as the sum of squared weighted residuals for each: 

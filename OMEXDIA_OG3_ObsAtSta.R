OBSatstaSSf <- function(sta){
  ...
  # List of variables that have to be retained
  varlimod<-c("NH3","DIC","SIO","PO4","TOC","TN","SiDet")
  
  # If we agree on Conventions we should be able to bypass those lines
  colnames(datadf)[which(colnames(datadf)=="OC")]<-"TOC"
  colnames(datadf)[which(colnames(datadf)=="ON")]<-"TN"
  colnames(datadf)[which(colnames(datadf)=="SiO2")]<-"SIO"
  colnames(datadf)[which(colnames(datadf)=="Bsi")]<-"SiDet"
  
  # remove data above SWI  
  bTM <-subset(datadf,MidDepth>0 & Station==sta,select=c("MidDepth","UpperDepth","LowerDepth",varlimod))
